...
// class _otpState extends State<otp> with CodeAutoFill {
              ...
              return Column(
                children: [
                  ...
                  Align(
                    alignment: Alignment.centerLeft,
                    child: Text(
                      widget.inWhichScreen == 'Email screen'
                          ? widget.email!.text
  ...
  otpcheck() async {
    if (_codeController.text.isNotEmpty) {
      ...
      if (widget.inWhichScreen == 'Email screen') {
        // Verify email OTP
        verifyEmailOtp();
      } else {
        ...
        if (send!.success == true) {
          ...
          if (send!.resData!.countryFullName != '') {
            await Hive.box(userdata).put(
                userCountryName, send!.resData!.countryFullName.toString());
          }
          ...
          showCustomToast(
              languageController.textTranslate('CANNOT VERIFY OTP'));
        }
      }
    } else {
      setState(() {
...
//============ SET DATA FIREBASE OPT RESPONSE DATA TO HIVE SAVE DATA =========================
  ...
  resendoTP() async {
    if (widget.inWhichScreen == 'Email screen') {
      if (widget.email!.text.isNotEmpty) {
        ...
        request.headers.addAll(headers);
        request.fields['email_id'] = widget.email!.text;
  ...
  // In _otpState class inside otp.dart
  verifyEmailOtp() async {
    if (_codeController.text.isNotEmpty) {
      setState(() {
        isLoading2 = true;
      });
      var uri = Uri.parse(apiHelper.verifyOtpEmail);
      var request = http.MultipartRequest("POST", uri);
      Map<String, String> headers = {
        "Accept": "application/json",
      };
      ...
      // Add fields based on your second screenshot
      request.fields['email_id'] = widget.email!.text;
      request.fields['otp'] = _codeController.text;
      request.fields['device_token'] = _fcmtoken;
      request.fields['one_signal_player_id'] =
      ...
      if (send!.success == true) {
        // Store user data in Hive (similar to phone verification)
        await Hive.box(userdata).put(authToken, send!.token.toString());
        await Hive.box(userdata)
            .put(userName, send!.resData!.userName.toString());
        await Hive.box(userdata)
        ...
        await Hive.box(userdata)
            .put(userImage, send!.resData!.profileImage.toString());
        await Hive.box(userdata).put(userEmail, widget.email!.text);
